...
public class StudentDao extends DBManager {
    ...
    public Student queryStudentByUsername(String username) {
        String sql = "select sid,sname,sphone,semail,age,ssex,QQNumber,password,username from stutbl where sname=?";
    ...
    public boolean updateCurri(Student student) {
        String sql = "update curritbl set cstudent=?,cname=?,cid=?,cteacher=?,currigrade=? where No=?";
        Object[] param = {student.getSname(), student.getCurriName(), student.getCurriId(), student.getCteacher(), student.getCgrade(), student.getNo()};
        return super.update(sql, param) > 0;
    }
    ...
    public Vector<Vector<Object>> queryCurri(String stuName, String curriName) {
        Student stu = new Student();
        stu.setSname(stuName);
        stu.setCurriName(curriName);

        String sql = "select * from curritbl where 1=1";

        List<Object> list = new ArrayList<>();
        if (stu.getSname() != null && !stu.getSname().equals("")) {
            sql += " and cstudent like ?";
            list.add("%" + stu.getSname() + "%");
        }
        if (stu.getCurriName() != null && !stu.getCurriName().equals("")) {
            sql += " and cname like ?";
            list.add("%" + stu.getCurriName() + "%");
        }
        ...
        try {
            while (rst.next()) {
                ...
                row.add(rst.getString("cstudent"));
                row.add(rst.getString("cname"));
                row.add(rst.getString("currigrade"));
                row.add(rst.getString("cid"));
                row.add(rst.getString("cteacher"));
    ...
    public Student queryCurriByCurriName(String Cname) {
        String sql = "select No,cstudent,cname,cid,cteacher,currigrade from curritbl where cname=?";
        String[] param = {Cname};
        ResultSet rst = super.query(sql, param);
        try {
            if (rst.next()) {
                ...
                student.setNo(rst.getInt("No"));
                student.setSname(rst.getString("cstudent"));
                student.setCurriName(rst.getString("cname"));
                student.setCurriId(rst.getString("cid"));
                student.setCteacher(rst.getString("cteacher"));
                student.setCgrade(rst.getInt("currigrade"));
    ...
    public Student queryCurriByName(String stuName) {
        String sql = "select No,cstudent,cname,cid,cteacher,currigrade from curritbl where cstudent=?";
    ...
    public Student queryStudentByBoth(String stuName,String curriName){
        String sql = "select No,cstudent,cname,cid,cteacher,currigrade from curritbl where cstudent=? and cname=?";
        String[] param = {stuName,curriName};
        ResultSet rst = super.query(sql, param);
        try {
            if (rst.next()) {
                Student student = new Student();
